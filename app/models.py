...
class Event(models.Model):
    ...
    * The amount of money already received that is not allocated for any item
    ...
    def save_from_form(self, POST):
      ...
      revenue = POST.getlist('item_revenue')

      self.item_set.all().delete()
      for name, quantity, price, funding, cat, rev in zip(names, quantities, prices_per_unit, funding_already_received, categories, revenue):
