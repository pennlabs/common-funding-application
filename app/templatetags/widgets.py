...
def itemlist_requester(is_revenue, items, funded):
    ...
    return render_to_string(
        "app/templatetags/itemlist-requester.html", context=new_context
    )
...
def application(user, event, form):
    ...
    new_context = {
        ...
        "funder_qas": [
            QA(
                question,
                get_or_none(FreeResponseAnswer, question=question, event=event),
            )
            for question in FreeResponseQuestion.objects.all()
        ],
        "eligibility_qas": [
            QA(question, get_or_none(EligibilityAnswer, question=question, event=event))
            for question in EligibilityQuestion.objects.all()
        ],
        "commonfreeresponse_qas": [
            QA(
                question,
                get_or_none(CommonFreeResponseAnswer, question=question, event=event),
            )
    ...
    if event is None:
        return render_to_string(
            "app/templatetags/application.html", context=new_context
        )
    ...
    if event.followup_needed or event.over:
        ...
        new_context["commonfollowup_qas"] = [
            QA(
                question,
                get_or_none(CommonFollowupAnswer, question=question, event=event),
            )
        ...
        new_context["followup_qas"] = [
            ...
            for question in FollowupQuestion.objects.filter(
